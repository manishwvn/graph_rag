...
## 🟡 10. Re-running the eval costs the full API budget every time

Judge verdicts and answers are not cached, so any rerun spends ~128 Groq calls
even when only the report formatting changed. This is what exhausted the daily
quota (issue 1).

**Action:** cache answers and verdicts keyed by (query, system, context hash);
add `--no-judge` for retrieval-only iterations.

## 🟡 11. RRF weighting is unvalidated

`chunk_ids_from_triples` scores a chunk as `sum(1/(rank+1))` over the triples
citing it. Standard reciprocal-rank fusion, but the constant was never tuned or
compared against alternatives (`1/(k+rank)`, uniform, rank-thresholded) on this
data.
...
## 🟡 13. `compare/` relies on implicit namespace packages

No `__init__.py` anywhere under `compare/`. Imports resolve only because the
repo root is on `sys.path` (via `pyproject.toml`'s
`[tool.pytest.ini_options] pythonpath`, or by running from the root).
`python compare/vector/pipeline_vector.py` directly would fail.

**Action:** either add the `__init__.py` files or drop the module-level entry
points that imply direct execution works.
...
## 🟡 16. Compare pipelines have no end-to-end test
